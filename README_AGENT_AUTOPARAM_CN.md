# V33 Agent 首选规则：规整模板 Primitive

除非用户明确要求保持原始复杂曲面或旧版冻结接口变形，Agent 对 `--fit-mode primitive` 应默认添加：
...
- 瓶子、罐子、动物、人物、机械零件：`regular`；
- 只有明确要求 constrained surface / frozen interface 局部变形时：`adaptive`；
- `regular` 下不要把 `--primitive-surface-*` 参数当作主要调参项，它不会使用 constrained surface；
- 规整模板接触使用 existing attachment face 或小 connector，不切割主体；
- 有视觉接触但 Hunyuan 网格存在小缝时，提高 `--primitive-contact-proximity-ratio`，通常使用 `0.015–0.03`；
- 出现大白色法兰/圆盘时，降低 `--primitive-regular-connector-max-face-area-ratio` 至 `0.03–0.06`，不要放大 interface 参数；
- `convex` 在 regular 模式中仅作为兜底；优先明确列出 `prism,frustum,cone,ellipsoid,box`。

详细变更见 `CHANGES_V33_REGULAR_TEMPLATE_PRIMITIVES.md`。
